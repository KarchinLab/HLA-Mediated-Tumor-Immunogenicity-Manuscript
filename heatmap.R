...
colnames(pMHC_matrix)=c("HLA-II","CD8 Tc1","CD4 Tmem",'Th1','Th2','Th17',"Tregs","NK","M1","M2","IDO1",'CXCL9','CXCL10','STAT1','IFNG',"PDCD1","CD274","CTLA4","LAG3","TIGIT",'HAVCR2')
rownames(pMHC_matrix)=c("ACC","BLCA","BRCA","CESC","CHOL","COAD","DLBC","ESCA","GBM","HNSC","KICH","KIRC","KIRP","LGG","LIHC","LUAD","LUSC","MESO","OV","PAAD","PCPG","PRAD","READ","SARC","SKCM","STAD","TGCT","THCA","THYM","UCEC","UCS","UVM")
...
annotation_row <- data.frame(row.names = c("ACC","BLCA","BRCA","CESC","CHOL","COAD","DLBC","ESCA","GBM","HNSC","KICH","KIRC","KIRP","LGG","LIHC","LUAD","LUSC","MESO","OV","PAAD","PCPG","PRAD","READ","SARC","SKCM","STAD","TGCT","THCA","THYM","UCEC","UCS","UVM","ACC1","BLCA1","BRCA1","CESC1","CHOL1","COAD1","DLBC1","ESCA1","GBM1","HNSC1","KICH1","KIRC1","KIRP1","LGG1","LIHC1","LUAD1","LUSC1","MESO1","OV1","PAAD1","PCPG1","PRAD1","READ1","SARC1","SKCM1","STAD1","TGCT1","THCA1","THYM1","UCEC1","UCS1","UVM1","ACC2","BLCA2","BRCA2","CESC2","CHOL2","COAD2","DLBC2","ESCA2","GBM2","HNSC2","KICH2","KIRC2","KIRP2","LGG2","LIHC2","LUAD2","LUSC2","MESO2","OV2","PAAD2","PCPG2","PRAD2","READ2","SARC2","SKCM2","STAD2","TGCT2","THCA2","THYM2","UCEC2","UCS2","UVM2","ACC3","BLCA3","BRCA3","CESC3","CHOL3","COAD3","DLBC3","ESCA3","GBM3","HNSC3","KICH3","KIRC3","KIRP3","LGG3","LIHC3","LUAD3","LUSC3","MESO3","OV3","PAAD3","PCPG3","PRAD3","READ3","SARC3","SKCM3","STAD3","TGCT3","THCA3","THYM3","UCEC3","UCS3","UVM3"), 
                            'II' = c(rep("HLA I", 32), rep("HLA II", 32), rep("TMB", 32), rep('Neoantigen Load', 32)))


annotation_colors = list(
  'I' = c("Immune Infiltration" = "lightsalmon2","Proinflammatory Genes" = 'deepskyblue3',"Immune Checkpoints" = 'olivedrab'),
  'II' = c("HLA I" = "dodgerblue3","HLA II" = 'goldenrod2',"TMB" = 'gray32', 'Neoantigen Load' = 'lightskyblue4')
)
...
out=pheatmap::pheatmap(data2, color = my_palette, cluster_cols=T,cluster_rows=F, annotation_colors = annotation_colors,
                   annotation_col=annotation_col, annotation_row=annotation_row, annotation_legend = T, gaps_col = c(9, 14),gaps_row = c(32, 64, 96),
...
# Figure S2A: spearman correlation between CYT and HLAI, HLAII, pMHC, TMB
...
p + geom_tile() + scale_fill_gradient2() + 
  ...
  labs(y=NULL, x=NULL, fill="Spearman's Rho") + coord_flip() + theme_bw() +
