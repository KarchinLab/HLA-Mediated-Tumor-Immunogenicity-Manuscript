...
# Figure S6J: HR plot, high HLAI vs. low HLA I expression ~ LOH
highHLAI_df=subset(df_HLALOH,df_HLALOH$high_HLAI == T)
lowHLAI_df=subset(df_HLALOH,df_HLALOH$high_HLAI == F)
...
ggplot(df, aes(x = boxOdds, y = HLA)) +
  ...
  scale_x_continuous(breaks = seq(0,7,1) ) +
  ylab("") +
  xlab("Odds ratio (log scale)") + ggtitle('TCGA BRCA, LUAD, SKCM Combined') +
  geom_vline(aes(xintercept = 1), size = .25, linetype = "dashed") + coord_cartesian(xlim = c(0, 2.5)) +
  annotate(geom = "text",label = c('HR=1.48\n*p = 0.014','HR=1.06\nns'),y =c(1,2), x = c(0.3,0.3),size=4)
...
BRCA_HLALOH$highCYT=BRCA_HLALOH$CYT > unname(quantile(BRCA_HLALOH$CYT,na.rm=TRUE)["50%"])
LUAD_HLALOH$highCYT=LUAD_HLALOH$CYT > unname(quantile(LUAD_HLALOH$CYT,na.rm=TRUE)["50%"])
SKCM_HLALOH$highCYT=SKCM_HLALOH$CYT > unname(quantile(SKCM_HLALOH$CYT,na.rm=TRUE)["50%"])

coxph(Surv(PFI_time_1, PFI_1) ~ highCYT, data = SKCM_HLALOH) %>% gtsummary::tbl_regression(exp = TRUE)

ggplot(SKCM_HLALOH %>% filter(!is.na(`HLA LOH`)), aes(x= `HLA LOH`, y=CYT)) + 
  geom_violin(aes(fill=`HLA LOH`)) + 
  scale_fill_manual(values=c("deepskyblue4", "firebrick")) + 
  stat_compare_means(label.x = 1, label.y = 12) +
  geom_boxplot(width=0.1) + 
...
df_highCYT=subset(LUAD_HLALOH,LUAD_HLALOH$highCYT==T)
piechart_df1=data.frame(c("LOH", "no LOH"),c(nrow(subset(df_highCYT,df_highCYT$`HLA LOH` == T)),nrow(subset(df_highCYT,df_highCYT$`HLA LOH` == F))))
colnames(piechart_df1)=c("HLALOH","ifLOH")
piechart_df1 <- piechart_df1 %>% 
  arrange(desc(HLALOH)) %>%
...
df_lowCYT=subset(LUAD_HLALOH,LUAD_HLALOH$highCYT==F)
piechart_df2=data.frame(c("LOH", "no LOH"),c(nrow(subset(df_lowCYT,df_lowCYT$`HLA LOH` == T)),nrow(subset(df_lowCYT,df_lowCYT$`HLA LOH` == F))))
colnames(piechart_df2)=c("HLALOH","ifLOH")
piechart_df2 <- piechart_df2 %>% 
  arrange(desc(HLALOH)) %>%
