...
# Figure S1A-B: see GTEx-figures.R
# immunogeneity section. Related to Figure 2 and S2.
# Figure 2A: see heatmap.R


# Figure S2A: see heatmap.R for correlation heatmap of comparison between TMB, neoantigens and HLA genes
...
# Figure 2SB-C: see 0711-UPDATE-highCYT-classifiers.ipynb
...
# Figure 3B: histogram of all tumor samples
...
# Figure S3G: HED correlation matrix with immune signatures 
...
# Figure 3D: all tumor samples; highHED: top 50% (ns); highHLAI: top 25%.
...
x<-c(0.62, 1.63, 2.67, 3.7, 4.71, 5.67)
...
ggplot(dat.m1, aes(x=immuneSubtype, y=value, fill=Tissue)) +
  ...
  stat_compare_means(aes(group = Tissue, label = ifelse( p < 2e-16, "p < 2e-16", 
                                                         paste0("p = ", ..p.format..))), label.x = 1, label.y =15, size=3) +
...
# Figure 4F, S4C: hazard ratio, see HR_plots.R
